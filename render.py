...
def teken_alles_pvp(bord, posities, owned_pos_speler1, owned_pos_speler2, gevangen_beurten, paused, UI, clock, player):
    for i in range(0, 10, 9):#witte hoeken
        pygame.draw.rect(bord.Screen.screen, (255, 255, 255), (80*i+75,130,80,90))
        pygame.draw.rect(bord.Screen.screen, (255, 255, 255), (80*i+75,580,80,90))
        bord.Screen.screen.blit(UI.go_to_jail_, (75,130))
    ...
    bord.Screen.screen.blit(player_0, (posities["speler1"]["x"], posities["speler1"]["y"]))
    bord.Screen.screen.blit(bot0, (posities["speler2"]["x"], posities["speler2"]["y"]))
    bord.Screen.screen.blit(font.render(f"Budget: {posities['speler1']['budget']}", True, (230,230,230)), (200, 220))
    bord.Screen.screen.blit(font.render(f"Eigendommen: {posities['speler1']['eigendom']}", True, (230,230,230)), (200,270))
    bord.Screen.screen.blit(font.render(f"Budget bot: {posities['speler2']['budget']}", True, (230,230,230)), (200, 320))
    bord.Screen.screen.blit(font.render(f"Eigendommen bot: {posities['speler2']['eigendom']}", True, (230,230,230)), (200,370))
    
    if gevangen_beurten["speler1"] > 0:
        bord.Screen.screen.blit(font.render(f"{player.speler1.naam}GEVANGENIS: {gevangen_beurten['speler1']} beurten", True, (255, 50, 50)), (200, 420))
    if gevangen_beurten["speler2"] > 0:
        bord.Screen.screen.blit(font.render(f"{player.speler2.naam} GEVANGENIS: {gevangen_beurten['speler2']} beurten", True, (255, 50, 50)), (200, 470))
    
    for pos in owned_pos_speler1:
        bord.Screen.screen.blit(UI.speler_owned, (pos["x"], pos["y"]))
        # Teken level als het bestaat
        if "level" in pos and pos["level"] > 0:
            bord.Screen.screen.blit(UI.small_font.render(str(pos["level"]), True, (255,255,255)), (pos["x"] + 14, pos["y"] + 9))

    for pos in owned_pos_speler2:
        bord.Screen.screen.blit(UI.bot_owned, (pos["x"], pos["y"]))
        # Teken level als het bestaat
        if "level" in pos and pos["level"] > 0:
            bord.Screen.screen.blit(UI.small_font.render(str(pos["level"]), True, (255,255,255)), (pos["x"] + 14, pos["y"] + 9))
